...
numbered rotation files so copied consensus messages cannot be replayed on the
fork. It also replaces the address book at the configured `p2p.addr_book_file`
path with an empty one and clears pending source-chain evidence from the
configured evidence database. Committed evidence records are preserved.
