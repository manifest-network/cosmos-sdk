...
1. Stop the source node and make a disposable copy of its home directory. The
   ...
   `p2p.addr_book_file` must be nonempty and name a file. Its parent directory, or
   nearest existing ancestor when parents are missing, must be writable by the
   process running conversion.
