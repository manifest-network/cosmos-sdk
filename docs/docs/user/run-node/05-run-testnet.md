...
The copied home must have committed consensus state and local blocks. For
`N >= 1`, conversion accepts these height arrangements:

| Application committed height | CometBFT saved state height | Blockstore height | Conversion result |
| --- | --- | --- | --- |
| `N` | `N` | `N` | Continue from committed height `N`. |
| `N` | `N` | `N+1` | Discard stored block `N+1` and continue from committed height `N`. |
| `N+1` | `N` | `N+1` | Recover CometBFT state from the committed block and its stored responses, then continue from committed height `N+1`. |

Preflight rejects homes without committed state or local blocks, unsupported
height gaps, and missing or inconsistent block data required for conversion.

In the second case, stored block `N+1` has not been committed by the application.
Conversion discards that source block; the first new fork block is at `N+1`.
To preserve the stored source block, recover the source node normally and stop
it cleanly before making the copy.

In the third case, conversion does not replay the recovered block's events.
Transaction and block indexes may therefore be incomplete, and event subscribers
do not receive those events from conversion. For complete copied indexes, recover
the source node normally and stop it cleanly before making the copy.
...
1. Stop the source node and make a disposable copy of its home directory. The
   ...
   the copy so you can restore it if conversion fails. Review the copied
   configuration for absolute filesystem paths, including address-book and
   consensus WAL paths. They are used as-is and are not confined to the copied
   home by `--home`; redirect them into the disposable copy before proceeding.
   The configured `p2p.addr_book_file` must be nonempty.
2. Install a fresh local consensus key in the copied home's configured private
   ...
   private key, and the address must match the public key. The command checks key
   completeness and consistency; ensuring the key is fresh is your responsibility.
   It requires this file to exist and does not generate or replace the key for you.
...
zero with no signature or sign bytes. After preflight succeeds, it first creates
any missing parent directories for the configured address-book path. It removes
the configured consensus WAL and its numbered rotation files so copied consensus
messages cannot be replayed on the fork. It also replaces the address book at the
configured `p2p.addr_book_file` path with an empty one and clears pending
source-chain evidence from the configured evidence database. Committed evidence
