...
| `N+1` | `N` | `N+1` | Recover CometBFT state at `N+1` from the committed block and its stored responses. |
...
at the configured `p2p.addr_book_file` path with an empty one and clears pending
